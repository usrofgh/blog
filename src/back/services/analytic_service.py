...
class AnalyticService:
    ...
    async def analytic_comments(db: AS, filters: AnalyticCommentFilterSchema) -> list[AnalyticCommentSchema]:
        stmt = select(
            ...
            func.count(case((CommentModel.is_blocked is True, 1), else_=None)).label("blocked_count"),
            func.count(case((CommentModel.is_blocked is False, 1), else_=None)).label("passed_count")
        ).where(
